...
<script>
    ...
    <h1 class="text-center p-3 bg-info-sutil text-info-emphasis">Cuerpo Directivo</h1>

    <div class="container-fluid row">
        <form class="border border-primary p-4 rounded col-4 p-3" method="POST" enctype="multipart/form-data">
            <?php
            include_once "modelo/conexion.php";
            include_once "controlador/registro_cdir.php";
            ?>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="trayectoria" name="trayectoria" required>
            </div>
        ...
        <div class="col-8 p-4">
            ...
            <table class="table table-bordered border-primary">
                ...
                <tbody>
                    ...
                    $sql = $conexion->query("SELECT * FROM directivo");
                    while ($datos = $sql->fetch_object()) { ?>
                        <tr>
                            <td><?= htmlspecialchars($datos->id, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars($datos->nombre, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars($datos->apellido, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars($datos->descripcion, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars($datos->trayectoria, ENT_QUOTES, 'UTF-8') ?></td>
                            <td>
                                <a href="vista/modificar_cdir.php?id=<?= htmlspecialchars($datos->id, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-small btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>
                                <a onclick="return confirmarEliminacion()" href="controlador/eliminar_cdir.php?id=<?= htmlspecialchars($datos->id, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-small btn-danger"><i class="fa-solid fa-trash-can"></i></a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
