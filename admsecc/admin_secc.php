...
<head>
    ...
    <title>Infodrive - Administración de Secciones</title>
...
<script>
    ...
    <h1 class="text-center p-3 bg-info-sutil text-info-emphasis">Administración de Secciones</h1>

    <div class="container-fluid row">
        <form class="border border-primary p-4 rounded col-4 p-3" method="POST" enctype="multipart/form-data">
            <?php
            include_once "modelo/conexion.php";
            include_once "controlador/registro_secc.php"; // Controlador para registrar secciones
            ?>
            <div class="mb-3">
        ...
        <div class="col-8 p-4">
            ...
            <table class="table table-bordered border-primary">
                ...
                <tbody>
                    <?php
                    include_once "modelo/conexion.php";
                    $sql = $conexion->query("SELECT * FROM secciones");
                    while ($datos = $sql->fetch_object()) { ?>
                        <tr>
                            <td><?= htmlspecialchars($datos->id, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars($datos->nombre, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars($datos->turno, ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars($datos->curso_id, ENT_QUOTES, 'UTF-8') ?></td>
                            <td>
                                <a href="vista/modificar_secc.php?id=<?= htmlspecialchars($datos->id, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-small btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>
                                <a onclick="return confirmarEliminacion()" href="controlador/eliminar_secc.php?id=<?= htmlspecialchars($datos->id, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-small btn-danger"><i class="fa-solid fa-trash-can"></i></a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
